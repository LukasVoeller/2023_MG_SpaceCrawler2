...
class CodeTable:
	...
	func get_entrie(index) -> CodeEntry:
		return self.entries.get(index, null)

	func _to_string() -> String:
		var result: String = 'CodeTable:\n'
		for id in self.entries:
			result += str(id) + ': ' + self.entries[id]._to_string() + '\n'
...
func decompress_lzw(code_stream_data: PackedByteArray, min_code_size: int, colors: PackedByteArray) -> PackedByteArray:
	...
	index_stream.append_array(code_table.get_entrie(code).sequence)
	# set PREVCODE = CODE
	...
	while true:
		...
		var code_entry: CodeEntry = code_table.get_entrie(code)
		if code_entry != null: # if YES
			...
			code_table.add(code_table.get_entrie(prevcode).add(K))
			# set PREVCODE = CODE
			prevcode = code
		else: # if NO
			# let K be the first index of {PREVCODE}
			var prevcode_entry: CodeEntry = code_table.get_entrie(prevcode)
