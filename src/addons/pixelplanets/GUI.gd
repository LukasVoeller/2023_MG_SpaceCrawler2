...
@onready var colorbutton_scene = preload("res://src/addons/pixelplanets/ColorPickerButton.tscn")
const GIFExporter = preload("res://src/addons/pixelplanets/addons/gdgifexporter/exporter.gd")
const MedianCutQuantization = preload("res://src/addons/pixelplanets/addons/gdgifexporter/quantization/median_cut.gd")

@onready var planets = {
	"Terran Wet": preload("res://src/addons/pixelplanets/Planets/Rivers/Rivers.tscn"),
	"Terran Dry": preload("res://src/addons/pixelplanets/Planets/DryTerran/DryTerran.tscn"),
	"Islands": preload("res://src/addons/pixelplanets/Planets/LandMasses/LandMasses.tscn"),
	"No atmosphere": preload("res://src/addons/pixelplanets/Planets/NoAtmosphere/NoAtmosphere.tscn"),
	"Gas giant 1": preload("res://src/addons/pixelplanets/Planets/GasPlanet/GasPlanet.tscn"),
	"Gas giant 2": preload("res://src/addons/pixelplanets/Planets/GasPlanetLayers/GasPlanetLayers.tscn"),
	"Ice World": preload("res://src/addons/pixelplanets/Planets/IceWorld/IceWorld.tscn"),
	"Lava World": preload("res://src/addons/pixelplanets/Planets/LavaWorld/LavaWorld.tscn"),
	"Asteroid": preload("res://src/addons/pixelplanets/Planets/Asteroids/Asteroid.tscn"),
	"Black Hole": preload("res://src/addons/pixelplanets/Planets/BlackHole/BlackHole.tscn"),
	"Galaxy": preload("res://src/addons/pixelplanets/Planets/Galaxy/Galaxy.tscn"),
	"Star": preload("res://src/addons/pixelplanets/Planets/Star/Star.tscn"),
}
var pixels = 100.0
var _scale = 1.0
var sd = 0
...
func save_image(img):
	if OS.get_name() == "HTML5" and OS.has_feature('JavaScript'):
		JavaScriptBridge.download_buffer(img.save_png_to_buffer(), String(sd)+".png", "image/png")
	else:
		if OS.get_name() == "OSX":
...
func export_gif(frames, frame_delay, progressbar):
	...
	if OS.get_name() != "HTML5" or !OS.has_feature('JavaScript'):
		var file
		if OS.get_name() == "OSX":
			file = FileAccess.open("user://%s.gif"%String(sd), FileAccess.WRITE)
		else:
			file = FileAccess.open("res://%s.gif"%String(sd), FileAccess.WRITE)
		file.store_buffer(exporter.export_file_data())
		file.close()
	else:
		var data = Array(exporter.export_file_data())
		JavaScriptBridge.download_buffer(data, String(sd)+".gif", "image/gif")
...
func _on_SearchPlanet_pressed():
	_seed_random()
	rng.randomize()
	
	var planet_type_rng = rng.randi_range(1, 6)
	
	if planet_type_rng == 1:
		_on_RandomizeColors_pressed()
		_create_new_planet(planets["Terran Wet"])
	elif planet_type_rng == 2:
		_on_RandomizeColors_pressed()
		_create_new_planet(planets["Terran Dry"])
	elif planet_type_rng == 3:
		_on_RandomizeColors_pressed()
		_create_new_planet(planets["Islands"])
	elif planet_type_rng == 4:
		_on_RandomizeColors_pressed()
		_create_new_planet(planets["Gas giant 1"])
	elif planet_type_rng == 5:
		_on_RandomizeColors_pressed()
		_create_new_planet(planets["Ice World"])
	elif planet_type_rng == 6:
		_on_RandomizeColors_pressed()
		_create_new_planet(planets["Lava World"])


func _on_SavePlanet_pressed():
	var planet = viewport_planet.get_child(0)
	var planet_colors = planet.get_colors()
	var planet_seed = sd
	
	get_parent().get_node("PlanetBar/V/PlanetSeed/Value").text = str(planet_seed)
	get_parent().get_node("PlanetBar/V/PlanetColors/Value").text = str(planet_colors)
...
func save_planet():
	var planet_seed = sd
	var planet_colors = viewport_planet.get_child(0).get_colors()
	...
	var save_game = FileAccess.open("user://saveplanets.save", FileAccess.WRITE)
