...
func change_colors_to_codes(image: Image,
		...
		transparency_color_index: int) -> PackedByteArray:
	false # image.lock() # TODOConverter40, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
	var image_data: PackedByteArray = image.get_data()
...
func add_frame_with_lci(image: Image, frame_delay: float) -> int:
	...
	var image_converted_to_codes: PackedByteArray = converter.new().get_similar_indexed_datas(image, last_color_table)
